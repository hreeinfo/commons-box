...
public class SafeRefMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
    ...
    /**
     * Create a new {@code SafeRefMap} instance.
     */
    public SafeRefMap() {
     ...
     */
    public SafeRefMap(int initialCapacity) {
     ...
     */
    public SafeRefMap(int initialCapacity, float loadFactor) {
     ...
     *                         write to the map
     */
    public SafeRefMap(int initialCapacity, int concurrencyLevel) {
     ...
     */
    public SafeRefMap(int initialCapacity, ReferenceType referenceType) {
     ...
     *                         write to the map
     */
    public SafeRefMap(int initialCapacity, float loadFactor, int concurrencyLevel) {
    ...
    public SafeRefMap(int initialCapacity, float loadFactor, int concurrencyLevel, ReferenceType referenceType) {
        if (!(initialCapacity >= 0)) throw AppError.error("Initial capacity must not be negative");
        if (!(loadFactor > 0f)) throw AppError.error("Load factor must be positive");
        if (!(concurrencyLevel > 0)) throw AppError.error("Concurrency level must be positive");
        if (referenceType == null) throw AppError.error("Reference type must not be null");
    ...
    /**
     * AppLogFactory method that returns the {@link ReferenceManager}.
    ...
    public boolean containsKey(Object key) {
        Reference<K, V> reference = getReference(key, Restructure.WHEN_NECESSARY);
        Entry<K, V> entry = (reference != null ? reference.get() : null);
        return (entry != null && Types.equals(entry.getKey(), key));
    }
    ...
    public boolean remove(Object key, final Object value) {
        return doTask(key, new Task<Boolean>(TaskOption.RESTRUCTURE_AFTER, TaskOption.SKIP_IF_EMPTY) {
            ...
            protected Boolean execute(Reference<K, V> reference, Entry<K, V> entry) {
                if (entry != null && Types.equals(entry.getValue(), value)) {
                    reference.release();
                    return true;
                }
                return false;
    ...
    public boolean replace(K key, final V oldValue, final V newValue) {
        return doTask(key, new Task<Boolean>(TaskOption.RESTRUCTURE_BEFORE, TaskOption.SKIP_IF_EMPTY) {
            @Override
            protected Boolean execute(Reference<K, V> reference, Entry<K, V> entry) {
                if (entry != null && Types.equals(entry.getValue(), oldValue)) {
    ...
    protected static final class Entry<K, V> implements Map.Entry<K, V> {
        ...
        public final boolean equals(Object other) {
            ...
            return (Types.equals(getKey(), otherEntry.getKey()) &&
                    Types.equals(getValue(), otherEntry.getValue()));
        }

        @Override
        public final int hashCode() {
            return (Types.hc(this.key) ^ Types.hc(this.value));
        }
    }
    ...
    private class EntrySet extends AbstractSet<Map.Entry<K, V>> {
        ...
        public boolean contains(Object o) {
            if (o != null && o instanceof Map.Entry<?, ?>) {
                ...
                Reference<K, V> reference = SafeRefMap.this.getReference(entry.getKey(), Restructure.NEVER);
                Entry<K, V> other = (reference != null ? reference.get() : null);
                if (other != null) {
                    return Types.equals(entry.getValue(), other.getValue());
                }
            }
            return false;
        }
        ...
        public boolean remove(Object o) {
            if (o instanceof Map.Entry<?, ?>) {
                ...
                return SafeRefMap.this.remove(entry.getKey(), entry.getValue());
            }
            return false;
        }
        ...
        @Override
        public int size() {
            return SafeRefMap.this.size();
        }

        @Override
        public void clear() {
            SafeRefMap.this.clear();
        }
    }
    ...
    private class EntryIterator implements Iterator<Map.Entry<K, V>> {
        ...
        private void moveToNextSegment() {
            ...
            if (this.segmentIndex < SafeRefMap.this.segments.length) {
                this.references = SafeRefMap.this.segments[this.segmentIndex].references;
                this.segmentIndex++;
            }
        }
        ...
        @Override
        public void remove() {
            if (this.last != null) SafeRefMap.this.remove(this.last.getKey());
        }
    }
    ...
    protected class ReferenceManager {
        ...
        /**
         * AppLogFactory method used to create a new {@link Reference}.
         *
        ...
        public Reference<K, V> createReference(Entry<K, V> entry, int hash, Reference<K, V> next) {
            if (SafeRefMap.this.referenceType == ReferenceType.WEAK) {
